...
import osureg.lib.resources as r
import cv2
import numpy as np
import pdal
...
class GeoData:
    ...
    def _calculate_normals(self) -> None:
        import open3d as o3d
        pc_o3d=o3d.geometry.PointCloud()
        pc_o3d.points = o3d.utility.Vector3dVector(self.point_cloud)
        pc_o3d.estimate_normals()
        self.normal_vectors = np.array(pc_o3d.normals)
    ...
    def _create_ortho(self) -> None:
        raise NotImplementedError
    
    def _gen_pc(self) -> None:
        raise NotImplementedError
...
class PointCloud(GeoData):
    ...
    def _create_ortho(self) -> None:
        """
        Converts the point cloud to meters and rasters it to a orthophoto.
        """
        tag = ["AOI", "Foundation"][int(self.fnd)]
        # self.logger.info(
        #     f"Extracting DSM from {tag}-{self.type.upper()} with resolution of: {self.resolution} meters"
    ...
    def _gen_pc(self) -> None:
        import open3d as o3d
        pc_o3d=o3d.io.read_point_cloud(self.file)
        xyz=np.array(pc_o3d.points)
        self.point_cloud = xyz
...
class Mesh(GeoData):
    ...
    def _create_ortho(self) -> None:
        ...
        #combine 
        # ortho_arr=[]
        # with rasterio.open("temp_ortho_r.tif") as data:
        #     r= data.read(1)
        #     ortho_arr.append(r)
        # with rasterio.open("temp_ortho_g.tif") as data:
        #     g= data.read(1)
        #     ortho_arr.append(g)
        # with rasterio.open("temp_ortho_b.tif") as data:
        #     b= data.read(1)
        #     ortho_arr.append(b)
        # self.ortho=np.array(ortho_arr).astype(np.uint8)
        # self.ortho=self.ortho.transpose(1,2,0)
        # cv2.imwrite("tmp_ortho.png",self.ortho)
    ...
    def _gen_pc(self) -> None:
        import open3d as o3d
        mesh_o3d=o3d.io.read_triangle_mesh(self.file)
        n_vertice=np.asarray(mesh_o3d.vertices).shape[0]
        n_faces=np.asarray(mesh_o3d.triangles).shape[0]
        if n_faces:
            pc_o3d=mesh_o3d.sample_points_uniformly(n_vertice*2)
            self.point_cloud=np.array(pc_o3d.points)
        else:
            self.point_cloud=np.array(mesh_o3d.vertices)
