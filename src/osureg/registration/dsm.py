...
class DsmRegistration:
    ...
    def putative_matches(self, matches: List[cv2.DMatch]) -> None:
        if len(matches) < 4:
            print(  f"{len(matches)} putative keypoint matches found (4 required). "
    ...
    def register(self) -> None:
        status=self.dsm_register()
        if not status:
            self.ortho_register()

    def dsm_register(self) -> None:
        """
        ...
        * Filter putative matches for those that conform to a common
        ...
        if len(self.fnd_kp) < 4:
            print(f"{len(self.fnd_kp)} keypoints were identiifed in the Foundation data"
        ...
        if len(self.aoi_kp) < 4:
            print(f"{len(self.aoi_kp)} keypoints were identiifed in the "
    ...
    def _filter_putative(self) -> None:
        ...
        if model is None:
            print( "ransac model not fitted, no inliers found. Consider tuning "
                "DSM_RANSAC_THRESHOLD or DSM_RANSAC_MAX_ITER")
            return False
        #self.logger.info(f"{np.sum(inliers)} keypoint matches found.")
        if np.sum(inliers) < 4:
            print("Less than four keypoint matches found.")
            return False

        T: np.ndarray = model.transform
        c = np.linalg.norm(T[:, 0])
        if c<=0.67 and c>=1.5:
            print("Solved scale difference between foundation and AOI data exceeds 50%")
            return False
