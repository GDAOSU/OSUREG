...
from src.osureg.g2a.preprocess import *
from scipy import stats
from src.osureg.g2a.mask2polygon import run_fit_main
import copy
import math
from src.osureg.g2a.lsd_deep import predict_lsd
from src.osureg.g2a.ply import read_ply
...
def line_based_matching_g2a_interface(src_path,ref_path,out_dir,progress=False,reg=False):
    ground_path=src_path
    drone_path=ref_path
    config=REG_CONFIG()
    ...
    np.savetxt(os.path.join(config.out_dir,'transformation.txt'),line_reg.T_g2a)
    
    source=o3d.io.read_point_cloud(ground_path)
    source_reg=source.transform(line_reg.T_g2a)
    o3d.io.write_point_cloud(os.path.join(out_dir,'registration_result.ply'),source_reg)
    print("Registration result saved to: ",os.path.join(out_dir,'registration_result.ply'))
    if progress:
        progress.advance(reg,100)
    return line_reg.T_g2a
...
def line_based_matching_g2f_interface(src_path,ref_path,out_dir,progress=False,reg=False):
    ground_path=src_path
    footprint=ref_path
    out_dir=out_dir
    config=REG_CONFIG()
    ...
    transformation=line_based_matching_g2f(config)
    if progress:
        progress.advance(reg,90)

    source=o3d.io.read_point_cloud(ground_path)
    source_reg=source.transform(transformation)
    o3d.io.write_point_cloud(os.path.join(out_dir,'registration_result.ply'),source_reg)
    print("Registration result saved to: ",os.path.join(out_dir,'registration_result.ply'))
    if progress:
        progress.advance(reg,100)
...
if __name__=="__main__": 
    ...
    # T=line_based_matching_sem(config)
    # print(T)
    ground_path=r'F:\tmp\g2a\ground.ply'
    drone_path=r'F:\tmp\g2a\air.ply'
    out_dir=r'F:\tmp\g2a'
    line_based_matching_g2a_interface(ground_path,drone_path,out_dir)

    #DEMO 2: Ground to Footprint
    ground_path=r'F:\tmp\ground.ply'
    footprint=r'F:\tmp\footprint.kml'
    out_dir=r'F:\tmp'
    #line_based_matching_g2f_interface(ground_path,footprint,out_dir)
